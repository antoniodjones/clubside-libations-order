-- First add guest checkout columns to orders table
ALTER TABLE public.orders 
ADD COLUMN guest_email TEXT,
ADD COLUMN guest_name TEXT,
ADD COLUMN guest_phone TEXT;

-- Update RLS policies to allow guest order tracking
DROP POLICY IF EXISTS "Users can view their own orders" ON public.orders;
DROP POLICY IF EXISTS "Users can create their own orders" ON public.orders;
DROP POLICY IF EXISTS "Users can update their own orders" ON public.orders;

-- New policy for viewing orders (authenticated users and guests)
...
-- Policy for creating orders (authenticated users and guests)
CREATE POLICY "Users can create their own orders" 
ON public.orders 
FOR INSERT 
WITH CHECK (
  auth.uid() = user_id OR 
  (auth.uid() IS NULL AND guest_email IS NOT NULL AND guest_email != '')
);

-- Policy for updating orders (authenticated users and guests via order ID)
...
-- Now add timestamp columns for each order status phase
ALTER TABLE public.orders 
